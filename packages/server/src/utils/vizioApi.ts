...
export class VizioAPI {
    ...
    try {
      ...
      logger.info('Preparing to send request', {
        ...
        headers: requiresAuth
          ? { AUTH: this.authToken ? `${this.authToken.substring(0, 5)}...` : 'null' }
          : {},
        endpoint,
      ...
      if (typedError?.response?.status === 500) {
              ...
              JSON.stringify(
                {
                  ...
                  response: {
                    status: typedError?.response?.status,
                    data: typedError?.response?.data
                      ? JSON.stringify(typedError?.response?.data, null, 2)
                      : null,
                  },
                },
                null,
        ...
        if (retryCount > 0) {
          ...
          logger.info(
            `Retrying ${method} request to ${endpoint} after ${delay}ms. Attempts remaining: ${retryCount}`
          );
          ...
          logger.error(
            `All retries failed for ${method} request to ${endpoint} - 500 errors persisted`
          );
        }
      }
        ...
        if (error.response) {
          ...
          if (!isAuthError) {
            logger.debug('TV API response error', {
              status,
              data:
                typeof responseData === 'string' ? responseData.substring(0, 100) : responseData,
          ...
          throw new Error(
            `Vizio API error: ${status} ${JSON.stringify(responseData).substring(0, 100)}`
          );
        } else if (error.request) {
          throw new Error(
            `Vizio TV connection error: No response received (TV might be off or unreachable)`
          );
        }
      }
  ...
  async initiatePairing(): Promise<string> {
    ...
    try {
      ...
      if (
        ...
        'PAIRING_REQ_TOKEN' in response.ITEM
      ) {
      ...
      if (
        ...
        response.ITEM &&
        typeof response.ITEM === 'object' &&
        'CHALLENGE_KEY' in response.ITEM
      ) {
    ...
    try {
      ...
      if (response && response.STATUS) {
        ...
        if (response.STATUS.RESULT === 'INVALID_PARAMETER') {
          throw new Error(
            'INVALID_PARAMETER: Missing or incorrect pairing parameters. Please restart the pairing process.'
          );
        }
      }
      ...
      if (response && response.ITEM && response.ITEM.AUTH_TOKEN) {
        ...
        logger.info(
          `Received valid auth token: ${authToken.substring(0, 5)}... (${authToken.length} chars)`
        );
        return authToken;
      }
      ...
      if (
        ...
        throw new Error(
          'Invalid PIN: The TV rejected the PIN you entered. Try again with the correct PIN.'
        );
      }
  ...
  /**
   * Get current power state using multiple methods for accuracy
   */
  async getPowerState(): Promise<boolean> {
    try {
      const response = await this.sendRequest('/state/device/power_mode');
      logger.debug('Power state response:', { response });

      // Handle different possible response formats
      let powerStateFromApi = false;
      if (response && response.ITEMS && response.ITEMS.length > 0) {
        powerStateFromApi = response.ITEMS[0].VALUE === 1;
      } else if (response && response.item && response.item.VALUE !== undefined) {
        powerStateFromApi = response.item.VALUE === 1;
      } else {
        logger.warn('Could not determine power state from response structure', { response });
        return false;
      }

      // If API says TV is off, trust it
      if (!powerStateFromApi) {
        logger.debug('TV API reports power is OFF');
        return false;
      }

      // If API says TV is on, verify with additional checks
      // Some Vizio TVs respond to API calls even in standby mode
      try {
        // Try to get current input - this often fails when TV is actually off
        const currentInput = await this.getCurrentInput();
        if (!currentInput || currentInput === 'Unknown') {
          logger.info('TV API reports ON but cannot get current input - likely in standby mode');
          return false;
        }

        // Try to get volume - another indicator that TV is actually on
        const volume = await this.getVolume();
        if (volume === 0 || volume === undefined) {
          logger.info('TV API reports ON but volume is 0 or undefined - likely in standby mode');
          return false;
        }

        logger.debug('TV API reports ON and verified with input/volume checks', {
          input: currentInput,
          volume: volume,
        });
        return true;
      } catch (verificationError) {
        logger.info('TV API reports ON but verification failed - TV likely in standby mode', {
          error: verificationError instanceof Error ? verificationError.message : 'Unknown error',
        });
        return false;
      }
    } catch (error: any) {
      // Network/connection errors usually mean TV is off or unreachable
      logger.debug('Error getting power state from TV', {
  ...
  async setVolume(level: number): Promise<void> {
    ...
    try {
      ...
      const currentVolumeResponse = await this.sendRequest(
      ...
      if (
        ...
        !currentVolumeResponse.ITEMS ||
        !currentVolumeResponse.ITEMS.length
      ) {
        throw new Error('Failed to retrieve current volume settings');
      }

      // Find the volume item to get its hashval
      const volumeItem = currentVolumeResponse.ITEMS.find(
        (item: any) => item.NAME === 'Volume' || item.CNAME === 'volume'
      );
  ...
  async getSpeakersStatus(): Promise<boolean> {
    try {
      ...
      const volumeResponse = await this.sendRequest(
        '/menu_native/dynamic/tv_settings/audio/volume'
      );
      ...
      for (const endpoint of possibleEndpoints) {
        try {
          ...
          if (response && response.ITEMS && response.ITEMS.length > 0) {
            ...
            const speakersItem = response.ITEMS.find(
              (item: any) =>
                item.NAME &&
  ...
  async setInput(input: string): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        const currentInputResponse = await this.sendRequest(
          '/menu_native/dynamic/tv_settings/devices/current_input'
        );

        if (
          !currentInputResponse ||
          !currentInputResponse.ITEMS ||
          !currentInputResponse.ITEMS.length
        ) {
        ...
        logger.warn(
          `Error setting input using documented API method for input ${normalizedInput}`,
          {
  ...
  async sendKeyPress(key: string, retryCount = 2): Promise<void> {
    ...
    try {
      ...
      if (KEY_CODE_MAP[actualKey] !== undefined && KEY_CODESET_MAP[actualKey] !== undefined) {
        ...
        logger.debug(
          `Sending key ${actualKey} with CODESET=${KEY_CODESET_MAP[actualKey]}, CODE=${KEY_CODE_MAP[actualKey]}`
        );
        ...
        logger.warn(
          `Key press ${actualKey} failed: ${response?.STATUS?.DETAIL || 'unknown error'}`
        );
      ...
      if (
        retryCount > 0 &&
        !(error instanceof Error && error.message.includes('Server error (500)'))
      ) {
            ...
            JSON.stringify(
              {
                request: {
                  ...
                  url: `${this.getBaseUrl()}/key_command/`,
                  data:
                    KEY_CODE_MAP[actualKey] !== undefined
      ...
      throw new Error(
        `TV remote command ${actualKey} failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async launchApp(appName: string): Promise<void> {
    ...
    logger.info(
      `Using app config for ${normalizedAppName}: NAME_SPACE=${appConfig.NAME_SPACE}, APP_ID=${appConfig.APP_ID}${appConfig.MESSAGE ? ', has custom MESSAGE' : ''}`
    );
  ...
  private mapAppIdToName(nameSpace: number, appId: string): string {
    ...
    // Find matching app
    const matchingApp = appMappings.find(app => app.nameSpace === nameSpace && app.appId === appId);
