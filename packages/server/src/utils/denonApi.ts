...
export class DenonAPI {
  ...
  private async sendRequest(
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          throw new Error(`Denon AVR API error: ${error.response.status} ${JSON.stringify(error.response.data)}`);
        } else if (error.request) {
          throw new Error(`Denon AVR connection error: No response received (AVR might be off or unreachable)`);
        }
      }
   ...
   */
  async navigate(direction: 'UP' | 'DOWN' | 'LEFT' | 'RIGHT' | 'SELECT' | 'MENU' | 'RETURN'): Promise<void> {
    const commandMap = {
...
export function createDenonAPIFromEnv(): DenonAPI {
